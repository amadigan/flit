...
func main() {
	...
	file, err := os.OpenFile(out, os.O_CREATE|os.O_TRUNC|os.O_RDWR, 0666)
	if err != nil {
		panic(err)
	}
	defer file.Close()
	...
	if err := exwriter.CloseRoot(extractor.EndDocument{Id: modId, Type: "end"}); err != nil {
		panic(err)
	}

	if err := exwriter.Close(); err != nil {
		panic(err)
	}
